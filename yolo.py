...
class YoloLayer(Layer):
    ...
    def call(self, x):
        ...
        self.batch_seen = self.batch_seen.assign(0.)        

        # compute grid factor and net factor
        grid_h      = tf.shape(y_true)[1]
        grid_w      = tf.shape(y_true)[2]
        grid_factor = tf.reshape(tf.cast([grid_w, grid_h],dtype=tf.float32), [1,1,1,1,2])
        ...
        Warm-up training
        """
        self.batch_seen = tf.compat.v1.assign_add(self.batch_seen, 1.)
